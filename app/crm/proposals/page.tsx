          ...
          <div key={proposal.id} className="flex items-center gap-3 px-4 py-3 transition hover:bg-gray-50 sm:gap-4 sm:px-5">
            ...
            <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
              <div className="text-right">
                <p className="text-xs font-bold text-gray-800 sm:text-sm">${(isProposalLocked(proposal) ? (proposal.acceptedPrice ?? proposal.total) : proposal.total).toLocaleString()}</p>
              ...
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold sm:px-2.5 sm:text-[11px] ${proposal.status === "Draft" ? "bg-gray-100 text-gray-600" : proposal.status === "Sent" ? "bg-blue-50 text-blue-700" : proposal.status === "Won" || proposal.status === "Signed" || proposal.status === "Signed Offline" ? "bg-emerald-50 text-emerald-700" : proposal.status === "Declined" ? "bg-red-50 text-red-600" : "bg-amber-50 text-amber-700"}`}>{proposal.status === "Approved" || proposal.status === "Viewed" ? `Viewed${(proposal.viewCount || 0) > 1 ? ` (${proposal.viewCount})` : ""}` : proposal.status}</span>
              <button type="button" onClick={(e) => { e.stopPropagation(); setPermDeleteTarget(proposal); setShowPermDeleteConfirm(true); }} className="hidden rounded-md px-2 py-1 text-[11px] font-medium text-gray-400 transition hover:bg-red-50 hover:text-red-600 sm:block">Delete</button>
