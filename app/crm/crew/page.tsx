          ...
          <aside className="h-full w-full max-w-2xl overflow-y-auto bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="space-y-5 p-5">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <label className="grid gap-2 rounded-lg bg-gray-50 p-4 text-xs font-bold uppercase text-gray-500">Schedule Date<input type="date" value={selectedJob.scheduleDate} onChange={(event) => { updateAssignment(selectedJob.id, { scheduleDate: event.target.value }); if (event.target.value) { void logCrewActivity({ jobId: selectedJob.id, jobName: selectedJob.name, actor: currentUserName, action: "Updated schedule", details: `Schedule date set to ${event.target.value}`, module: "Crew Workflow" }); const startISO = toArizonaISO(event.target.value); const endISO = new Date(new Date(startISO).getTime() + 2 * 60 * 60 * 1000).toISOString(); void syncJobToCalendar(selectedJob.id, { title: `${selectedJob.roofType || "Roofing"} — ${selectedJob.name}`, description: `${selectedJob.roofType || "Roofing"} job for ${selectedJob.name}`, start_time: startISO, end_time: endISO, all_day: true, location: formatAddress(selectedJob), color: "#f97316", assigned_to: selectedJob.assignedCrew.join(", "), customer_name: selectedJob.name, customer_phone: selectedJob.phone || "", job_kind: selectedJob.roofType || "Roofing", created_by: currentUserName || "Office" }); } }} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-bold normal-case text-gray-800 outline-none" /></label>
