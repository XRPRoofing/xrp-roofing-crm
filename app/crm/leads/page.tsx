                    ...
                    return (
                      <div key={type} className={`border-t ${slotIndex === 0 ? "border-slate-700" : border}`}>
                        ...
                        <div className="relative w-full" style={{ aspectRatio: "16/9" }}>
                          {latest?.dataUrl ? (
                            <>
                              ...
                              <label className={`absolute inset-0 flex cursor-pointer items-center justify-center bg-black/0 opacity-0 transition hover:bg-black/40 hover:opacity-100 ${fileBusy ? "pointer-events-none" : ""}`}>
                                <span className="rounded-2xl bg-white/90 px-4 py-2 text-xs font-black text-slate-900">Tap to replace</span>
                                <input type="file" accept="image/*" capture="environment" className="hidden" disabled={fileBusy} onChange={(event) => { const input = event.currentTarget; void handleJobFileUpload(type, input.files).finally(() => { input.value = ""; }); }} />
                              </label>
                            </>
                          ) : (
                            <label className={`flex h-full w-full cursor-pointer flex-col items-center justify-center gap-3 ${addBg} transition ${fileBusy ? "pointer-events-none opacity-60" : ""}`}>
                              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white/10">
                                <Camera className="h-7 w-7 text-slate-400" />
                              </div>
                              <span className="text-sm font-black text-slate-400">Add {label.charAt(0) + label.slice(1).toLowerCase()} Photo</span>
                              <input type="file" accept="image/*" capture="environment" className="hidden" disabled={fileBusy} onChange={(event) => { const input = event.currentTarget; void handleJobFileUpload(type, input.files).finally(() => { input.value = ""; }); }} />
                            </label>
                          )}
                          {/* Label badge */}
                        ...
                        <div className="grid grid-cols-2 gap-px bg-slate-700">
                          <label className={`flex cursor-pointer items-center justify-center gap-2 py-3 text-xs font-black text-white transition ${camBg} ${fileBusy ? "pointer-events-none opacity-60" : ""}`}>
