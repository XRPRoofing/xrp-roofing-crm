...
const GROUP_COLORS: Record<string, string> = {
  ...
  "Review Requests":        "yellow",
  "Internal Notifications": "purple",
  "Calendar & Scheduling":  "emerald",
};
...
function colorClasses(color: string) {
  ...
  if (color === "yellow")  return { badge: "bg-yellow-50 text-yellow-700 border-yellow-100", dot: "bg-yellow-500", icon: "bg-yellow-100 text-yellow-600", ring: "ring-yellow-200" };
  if (color === "purple")  return { badge: "bg-purple-50 text-purple-700 border-purple-100", dot: "bg-purple-500", icon: "bg-purple-100 text-purple-600", ring: "ring-purple-200" };
  if (color === "emerald") return { badge: "bg-emerald-50 text-emerald-700 border-emerald-100", dot: "bg-emerald-500", icon: "bg-emerald-100 text-emerald-600", ring: "ring-emerald-200" };
...
export default function AutomationsPage() {
  ...
  const logStatusColor = (s: string) =>
    s === "sent" ? "text-emerald-700 bg-emerald-50" : s === "failed" ? "text-red-700 bg-red-50" : "text-slate-600 bg-slate-100";
        ...
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <span className="flex items-center gap-1.5 rounded-full bg-emerald-500/20 px-3 py-1.5 text-xs font-black uppercase tracking-wider text-emerald-200 ring-1 ring-emerald-400/30">
          ...
          <section key={group} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-5 py-4">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => enableAllInGroup(group, true)} className="rounded-xl px-3 py-1.5 text-[10px] font-black text-emerald-700 hover:bg-emerald-50">Enable All</button>
                  ...
                  return (
                    <div key={id} className={`px-5 py-4 transition ${rec.enabled ? "" : "opacity-60"}`}>
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div className="flex shrink-0 flex-wrap items-center gap-3">
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1.5 rounded-xl border px-2.5 py-1.5 text-[11px] font-black transition ${rec.channels.sms ? "border-purple-200 bg-purple-50 text-purple-700" : "border-slate-200 bg-white text-slate-400"}`}
                          >
                          ...
                          <button
                            ...
                            className="flex items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 text-[11px] font-black text-emerald-700 hover:bg-emerald-100 disabled:opacity-50"
                          >
                      ...
                      {testResult && testingId === null && editingId !== id && (
                        <p className={`mt-2 rounded-xl px-3 py-2 text-xs font-bold ${testResult.startsWith("✓") ? "bg-emerald-50 text-emerald-700" : testResult.startsWith("⚠") ? "bg-yellow-50 text-yellow-700" : "bg-red-50 text-red-700"}`}>
                          {testResult}
                        </p>
                      )}
