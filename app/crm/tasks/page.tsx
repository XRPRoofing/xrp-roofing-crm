...
import { CheckSquare, ChevronDown, ChevronRight, Clock, Star, ThumbsDown, ThumbsUp, X, Zap } from "lucide-react";
import {
...
export default function TasksPage() {
  ...
  const refresh = () => setTasks(readOfficeTasks());

  useEffect(() => {
    window.addEventListener("crm-office-tasks-updated", refresh);
    window.addEventListener("storage", refresh);
    return () => {
      window.removeEventListener("crm-office-tasks-updated", refresh);
      window.removeEventListener("storage", refresh);
    };
  }, []);

  // Auto-refresh every 15s for cross-device sync
  useEffect(() => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-500">Auto-synced with Jobs, Invoices &amp; Reviews. {tasks.length} total tasks.</p>
        </div>
        {filterStatus && (
          <button onClick={() => setFilterStatus(null)} className="flex items-center gap-2 rounded-2xl bg-blue-600 px-4 py-2 text-sm font-black text-white">
            <X className="h-4 w-4" /> Clear filter: {filterStatus}
          </button>
        )}
      </div>
