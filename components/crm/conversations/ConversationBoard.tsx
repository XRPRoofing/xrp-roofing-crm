...
export default function ConversationBoard() {
  ...
  return (
    <div className="-mx-3 -my-4 flex min-h-[calc(100vh-5rem)] flex-1 flex-col overflow-x-clip bg-gray-100 px-3 py-4 font-sans sm:-mx-5 sm:px-5 xl:h-[calc(100vh-8.25rem)] xl:max-h-[calc(100vh-8.25rem)] xl:min-h-0 xl:overflow-hidden">
      ...
      {isActiveCall && (
        <div className="sticky top-20 z-40 mb-4 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-blue-200 bg-blue-600 px-3 py-2.5 text-white shadow-sm sm:gap-3 sm:px-4 sm:py-3">
          <div className="flex min-w-0 items-center gap-2 sm:gap-3"><span className="h-2.5 w-2.5 shrink-0 rounded-full bg-blue-300" /><span className="truncate text-sm font-semibold">Active call with {matchedDialContact?.name || dialNumber}</span><Badge tone="green"><Clock className="mr-1 h-3 w-3" />{isHeld ? "Held" : "Live"}</Badge></div>
          <div className="flex shrink-0 gap-1.5 sm:gap-2"><Button variant="ghost" className="text-white hover:bg-blue-500" onClick={handleMuteCall}><Mic className="mr-1 h-3 w-3" />{isMuted ? "Unmute" : "Mute"}</Button><Button variant="ghost" className="hidden text-white hover:bg-blue-500 sm:inline-flex" onClick={() => { setIsDialerOpen(true); setIsDialerMinimized(false); }}>Notes / Forward</Button><button onClick={handleEndCall} className="inline-flex items-center rounded-lg bg-gray-950 px-3 py-2 text-sm font-semibold text-white transition hover:bg-gray-800"><PhoneOff className="mr-1.5 h-4 w-4" />End</button></div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 p-4" onClick={() => setScheduleOpen(false)}>
          <form onClick={(event) => event.stopPropagation()} onSubmit={handleSaveSchedule} className="flex max-h-[88vh] w-full max-w-md flex-col overflow-hidden rounded-lg bg-white shadow-xl">
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-5 py-4">
              ...
              <div className="grid gap-1"><div className="flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Notes</span><AiWriteButton getText={() => scheduleForm.notes} onReplace={(t) => setScheduleForm((form) => ({ ...form, notes: t }))} context="appointment notes for a roofing job" /></div><textarea value={scheduleForm.notes} onChange={(event) => setScheduleForm((form) => ({ ...form, notes: event.target.value }))} rows={3} className="resize-none rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm outline-none focus:border-blue-300 focus:bg-white" placeholder="Optional details" /></div>
